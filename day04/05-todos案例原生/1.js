...
function updateData(id, isDone) {
  Data[id].isDone = isDone;
}
...
function showData(myData) {
  ...
  let newData = Data.map(function (item) {
    ...
    let str = `<li>
              <label>
                <input type="checkbox" ${item.isDone?'checked':''}/>
                <span class=${item.isDone? 'done' : ''} data-id=${item.id}>${item.isDo}</span>
              </label >
              <button class="btn btn-danger">删除</button>
            </li>`
    return str;
...
function clickMain() {
  ...
  oMain.onclick = function (e) {
    ...
    if (e.target.nodeName == 'INPUT') {
      ...
      // }
      updateData(e.target.id, !(e.target.checked));
    }
    //点击影响checked
    isAllChecked()
  }
